...
namespace GraphProcessing
{
	...
	public:
		...
		void walker(T t, Sptr<V> pNode, P p, std::vector<std::string>& trace, bool walkAll = false);
	private:
	...
	*/
	template<typename V>
	template<typename T, typename P>
	void Graph<V>::walker(T t, Sptr<V> pNode, P p, std::vector<std::string>& trace, bool walkAll)
	{
...
void DFS_r_ltr(Sptr<V> pNode, P p, std::vector<std::string>& trace)
{
	if (pNode->marked())
		return;
	pNode->mark();
	trace.push_back(pNode->name());
	p(pNode);
	//std::cout << "\n  " << pNode->name();
	for (auto pChild : pNode->children())
		DFS_r_ltr(pChild, p, trace);
}
