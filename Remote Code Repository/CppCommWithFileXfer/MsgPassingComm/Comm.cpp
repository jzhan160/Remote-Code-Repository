...
bool Sender::sendFile(Message msg)
{
  if (!msg.containsKey("file"))
    return false;
  std::string sendFilePath = msg.attributes()["source"];
  std::string fileSpec = sendFilePath + "/" + msg.value("file");
  //FileSystem::Path path;
  //std::cout << path.getFullFileSpec(fileSpec)<<"\n";
...
public:
  ...
  */
  ...
  {
    std::string fileName = msg.value("toFile");
	std::string saveFilePath = msg.attributes()["destination"];
    std::string fileSpec = saveFilePath + "/" + fileName;
	//FileSystem::Path path;
	//std::cout << "\n =============================="<<path.getFullFileSpec(fileSpec) << "\n";
